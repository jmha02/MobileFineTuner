...
static void print_usage(const char* prog) {
    cerr << "Usage: " << prog << " [options]\n"
         ...
         << "  --lora_out PATH          LoRA output path\n"
         << "  --resume_from PATH       Resume from checkpoint\n"
         << "  --eval_out PATH          Evaluation output (JSONL)\n"
         << "  --epochs N               Epochs (overrides steps)\n"
         ...
         << "  --clip_grad_norm F       Gradient clip threshold\n"
         << "  --lora_dropout F         LoRA dropout rate\n"
         << "  --data_fraction F        Data fraction (0.0-1.0]\n"
         << "  --log_interval N         Log interval\n"
         << "  --eval_interval N        Eval interval\n"
         << "  --eval_batches N         Eval batches\n"
         << "  --eval_batch_size N      Eval batch size\n"
         << "  --save_every N           Checkpoint save interval\n"
         << "  --ema_beta F             EMA smoothing factor\n"
         << "  --seed N                 Random seed\n"
         << "  --shard_enable           Enable parameter sharding/offload\n"
         << "  --shard_dir PATH         Shard file directory\n"
         << "  --shard_budget_mb N      Resident memory budget (MB)\n"
         << "  --shard_fp16_disk 0|1    FP16 quantize on disk\n";
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        cout << "[Config]" << endl;
        // Compile-time BLAS toggle (informational; kernel usage depends on backend implementation)
#if defined(USE_BLAS)
        ...
        // 1) Build model and load pretrained weights
        cout << "\n[1/6] Loading pretrained model..." << endl;
        GPT2Config cfg;
        try {
            cfg = GPT2Config::from_pretrained(args.pretrained_dir);
            cout << "  ✓ Loaded GPT-2 config: layers=" << cfg.n_layer
                 << ", hidden=" << cfg.n_embd
                 << ", heads=" << cfg.n_head << endl;
        } catch (const std::exception& e) {
            cout << "  ⚠️ Failed to read config, using built-in defaults: " << e.what() << endl;
        }
        if (args.seq_len > cfg.n_positions) {
            cout << "  ⚠️ seq_len(" << args.seq_len << ") exceeds n_positions("
                 << cfg.n_positions << "), clamped" << endl;
        ...
        // In GPT-2 safetensors, linear weights are already [in, out] (e.g. [768, 2304]); no transpose needed.
        // To avoid runtime transpose overhead, disable load-time transpose here.
        SafeTensorsLoadOptions load_opts;
        load_opts.transpose_linear = false;
        load_opts.verbose = false;        // Reduce load logs during training
        ...
        // Bind sharder and register weights
        if (sharder) {
            model.set_parameter_sharder(sharder.get());
            // keep_in_memory=false: release memory immediately after registration (keep only disk copy),
            // relying on require() during forward/backward for on-demand loading.
            // This lowers initial peak memory, with runtime memory controlled by the budget.
            ...
            // Free temporary pretrained weight copies to avoid coexistence with shards raising peak memory.
            loaded.clear();
            loaded.rehash(0);
            MemoryManager::instance().force_cleanup();
        }

        // 2) Initialize LoRA and inject/restore
        cout << "\n[2/6] Inject/restore LoRA..." << endl;
        model.init_lora_modules();

        // Utility: clear existing LoRA slices to avoid duplicate attachment.
        auto clear_all_lora = [&]() {
        ...
        if (!args.resume_from.empty()) {
            cout << "  ↪ Resume file detected; restoring from checkpoint: " << args.resume_from << endl;
            clear_all_lora();
            auto lora_state = LoraSaver::load_safetensors(args.resume_from);
            LoraSaver::attach_from_state(model, lora_state);
            // After restore, refresh trainable parameters and ensure they are trainable.
            trainable = model.get_lora_parameters();
            for (auto& p : trainable) { p->set_requires_grad(true); p->zero_grad(); }
            cout << "  ✓ Loaded and bound LoRA weights from checkpoint (trainable=" << trainable.size() << ")" << endl;
            ...
            cout << "  ✓ LoRA injection complete: " << trainable.size() << " trainable parameters" << endl;
        ...
        cout << "  ✓ Validation set: " << valid_dataset.num_sequences() << " sequences" << endl;
        ...
        auto lr_schedule = [&](int step) -> float {
            ...
            // Avoid non-standard M_PI macro for portability.
        ...
        auto evaluate_valid = [&]() -> float {
            // Disable Autograd during eval; restore and clear graph on exit.
            struct NoGradGuard {
        ...
        for (int step = start_step; step < args.steps; ++step) {
            ...
            for (int acc = 0; acc < accum; ++acc) {
                ...
                // Option A: scale loss by 1/accum each micro-step, then backprop.
                {
            ...
            // Periodically clean free blocks (every 50 steps by default to reduce overhead).
            if (((step + 1) % 50) == 0) {
                MemoryManager::instance().force_cleanup();
            }
            // Every 100 steps, release free blocks back to OS (stronger release to prevent RSS creep).
            if (((step + 1) % 100) == 0) {
            ...
            // Print memory stats every 100 steps (monitor total_allocated / total_in_use).
            if ((step + 1) % 100 == 0) {
            ...
            if (args.save_every > 0 && (step + 1) % args.save_every == 0 && !args.lora_out.empty()) {
                ...
                if (has_active_lora_spec) {
                    ...
                    cout << "  ⚠️ No LoRA config detected; skipping save" << endl;
                }
            }
        ...
        // 6) Save final LoRA
        cout << "\n[6/6] Saving final LoRA weights..." << endl;
        if (!args.lora_out.empty()) {
            if (has_active_lora_spec) {
                ...
                cout << "  ⚠️ No LoRA weights to save; skipped" << endl;
            }
        }
